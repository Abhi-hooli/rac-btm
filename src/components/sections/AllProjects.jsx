...
function exportToPDF(projects, dateRange) {
  const totals = projects.reduce((acc, p) => ({
    ...
    contributions:    acc.contributions    + (p.cashContributions || p.contributions || 0),
    peopleImpacted:   acc.peopleImpacted   + (p.beneficiaries     || 0),
    participants:     acc.participants     + (p.participants      || 0),
    volunteers:       acc.volunteers       + (p.volunteers        || 0),
  }), { volunteerHours: 0, contributions: 0, peopleImpacted: 0, participants: 0, volunteers: 0 })
...
<div class="summary">
  ...
  <div class="summary-card"><div class="num">${totals.beneficiaries || totals.peopleImpacted.toLocaleString()}</div><div class="label">Beneficiaries</div></div>
  ...
  <tbody>
    ${projects.map((p, i) => {
      ...
      return `<tr>
        ...
        <td class="dates">${start}${start && end ? '<br>→<br>' : ''}${end || ''}</td>
        <td style="font-size:11px">${p.venue || '—'}</td>
        <td style="font-size:11px">
          ${p.participants  ? `👥 ${p.participants} participants<br>`  : ''}
          ${p.beneficiaries ? `🎯 ${p.beneficiaries} beneficiaries<br>` : ''}
          ${p.volunteers    ? `🙋 ${p.volunteers} volunteers<br>`       : ''}
          ${p.volunteerHours ? `⏱ ${p.volunteerHours} hrs<br>`         : ''}
          ${p.cashContributions ? `💰 ₹${p.cashContributions}<br>`     : ''}
        </td>
      </tr>`
    }).join('')}
  </tbody>
...
function exportToCSV(projects) {
  const headers = [
    ...
    'Avenue', 'Area of Focus',
    'Host Club', 'Partner Clubs', 'External Partners',
    'Expenses (₹)', 'Cash Contributions (₹)', 'In-Kind Contributions (₹)',
    'Participants', 'Beneficiaries', 'Volunteers', 'Volunteer Hours',
  ]
  const rows = projects.map(p => [
    ...
    p.expenses           || 0,
    p.cashContributions  || 0,
    p.inKindContributions|| 0,
    p.participants       || 0,
    p.beneficiaries      || 0,
    p.volunteers         || 0,
    p.volunteerHours     || 0,
...
export default function AllProjects({ isAdmin, onBack }) {
  ...
  const handleEdit = (p) => {
    setForm({
      title:               p.title               || '',
      activityType:        p.activityType         || activityTypes[0],
      venue:               p.venue                || '',
      startDate:           p.startDate            || '',
      endDate:             p.endDate              || '',
      avenue:              p.avenue               || rotaractAvenues[0],
      areaOfFocus:         p.areaOfFocus          || areasOfFocus[0],
      hostClub:            p.hostClub             || '',
      partnerClubs:        p.partnerClubs         || '',
      externalPartners:    p.externalPartners      || '',
      expenses:            p.expenses             || 0,
      cashContributions:   p.cashContributions    || 0,
      inKindContributions: p.inKindContributions  || 0,
      ...
      image:               p.image                || '',
      color:               p.color                || colorOptions[0].value,
  ...
  const handleExport = (asPDF = false) => {
    ...
    if (filterValue !== 'All') {
      out = out.filter(p =>
        p.avenue === filterValue || p.areaOfFocus === filterValue || p.activityType === filterValue
      )
    }
          ...
          {isAdmin && showForm && (
            ...
            >
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <input className={`${inputClass} md:col-span-2`} placeholder="Project Title *"
                ...
                <input className={inputClass} placeholder="Venue (e.g. BTM / Online – Google Meet)"
              ...
              <div className="grid md:grid-cols-3 gap-4 mb-6">
                <input className={inputClass} placeholder="Host Club"
                  value={form.hostClub} onChange={e => set('hostClub', e.target.value)} />
                <input className={inputClass} placeholder="Partner Clubs (comma-separated)"
                  value={form.partnerClubs} onChange={e => set('partnerClubs', e.target.value)} />
                <input className={inputClass} placeholder="External Partners / NGOs"
                  value={form.externalPartners} onChange={e => set('externalPartners', e.target.value)} />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-4 mb-6">
                <input className={inputClass} placeholder="Image URL"
                ...
                <textarea className={`${inputClass} md:col-span-2`} rows={3}
                  placeholder="Description (shown on project card)"
              ...
              <div className="mb-6">
                <Toggle value={form.featured} onChange={v => set('featured', v)}
                  ...
                  sub="Show in homepage Featured Projects section (max 4)" />
              </div>
                ...
                <div className="absolute inset-0 p-5 flex flex-col justify-end">
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {project.activityType && (
                      <span className="inline-block px-2.5 py-0.5 text-xs font-semibold bg-black/30 backdrop-blur-sm rounded-full text-white">
                        {project.activityType}
                      </span>
                    )}
                    {project.avenue && (
