...
const footerLinks = {
  'Quick Links': [
    ...
    { label: 'Join Rotaract', page: 'joinForm' },
  ],
  'Connect': [
...
export default function Footer({ goToPage }) {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      // Deterministic doc ID (the email itself) instead of a duplicate-check
      // read — `subscribers` requires auth to read, so an anonymous visitor
      // can't query it. Firestore rules distinguish create (public) vs update
      ...
      if (err.code === 'permission-denied') { setSubState('duplicate'); return }
      setSubState('error')
    }
  }
